...
/**
 * Reads documents from a single file partition and converts each to a Spark {@link InternalRow}.
 *
 * <p>Supports two file formats:
 *
 * <ul>
 *   <li><b>BSON</b> (.bson) — binary, concatenated BSON documents
 *   <li><b>EJSON</b> (.json) — line-delimited Extended JSON (MongoDB $out format)
 * </ul>
 */
public final class BsonPartitionReader implements PartitionReader<InternalRow> {
  ...
  public BsonPartitionReader(
    ...
    if (isBsonFormat) {
      ...
      this.jsonReader =
          new BufferedReader(new InputStreamReader(rawStream, StandardCharsets.UTF_8));
    }
  }
