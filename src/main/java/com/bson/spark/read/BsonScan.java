...
/**
 * Plans the input partitions for a BSON/EJSON file scan. Each file
 * becomes one partition for maximum parallelism.
 *
 * <p>Supported file extensions:
 * <ul>
 *   <li>{@code .bson}, {@code .bson.gz} — binary BSON format</li>
 *   <li>{@code .json}, {@code .json.gz} — line-delimited EJSON (Extended JSON)</li>
 * </ul>
 */
public final class BsonScan implements Scan, Batch {
    ...
    public InputPartition[] planInputPartitions() {
        ...
        if (partitions.isEmpty()) {
            throw new RuntimeException(
                    "No BSON/JSON files found at: " + readOptions.paths());
        }
    ...
    private void collectFiles(
            FileSystem fs, Path path, List<InputPartition> partitions) throws IOException {
        ...
        if (status.isDirectory()) {
            ...
            for (FileStatus child : children) {
                if (child.isDirectory()) {
                    ...
                    partitions.add(new BsonPartition(
                            child.getPath().toString(), child.getLen()));
                }
            }
