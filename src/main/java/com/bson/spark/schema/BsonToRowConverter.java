...
/**
 * Converts a {@link BsonDocument} to a Spark {@link InternalRow} based on
 * the provided schema.
 *
 * <p>This is the core of the DataSource — it maps BSON typed values
 * directly to Spark's internal representation without any intermediate
 * JSON serialization step.
 *
 * <p>Type mapping follows the declared schema. If the schema says
 * {@code StringType} for a field that contains an ObjectId, the ObjectId
 * is converted to its hex string. If the schema says {@code TimestampType},
 * the BSON datetime is converted to epoch microseconds. The schema drives
 * the conversion.
 */
public final class BsonToRowConverter implements Serializable {
    ...
    public BsonToRowConverter(StructType schema, BsonReadOptions options) {
        ...
        this.hasCorruptRecordColumn = !corruptRecordColumn.isEmpty()
    ...
    private InternalRow convertToStruct(
        ...
        if (!bsonValue.isDocument()) {
            throw new BsonConversionException(
                    String.format("Expected document for field '%s', got %s",
                            fieldName, bsonValue.getBsonType()));
        }
    ...
    private GenericArrayData convertToArray(
        ...
        if (!bsonValue.isArray()) {
            throw new BsonConversionException(
                    String.format("Expected array for field '%s', got %s",
                            fieldName, bsonValue.getBsonType()));
        }
        ...
        for (int i = 0; i < bsonArray.size(); i++) {
            ...
            if (element.isNull()) {
                ...
                elements[i] = convertValue(
                        fieldName + "[" + i + "]", elementType, element);
            }
        }
    ...
    private ArrayBasedMapData convertToMap(
            String fieldName, MapType mapType, BsonValue bsonValue) {
        if (!bsonValue.isDocument()) {
            throw new BsonConversionException(
                    String.format("Expected document for map field '%s', got %s",
                            fieldName, bsonValue.getBsonType()));
        }
        ...
        for (Map.Entry<String, BsonValue> entry : mapDoc.entrySet()) {
            ...
            if (val.isNull()) {
                ...
                values[i] = convertValue(
                        fieldName + "." + entry.getKey(), valueType, val);
            }
            i++;
        }
    ...
    private long convertToLong(BsonValue bsonValue) {
        switch (bsonValue.getBsonType()) {
            ...
            default:
                throw new BsonConversionException(
                        "Cannot convert " + bsonValue.getBsonType() + " to long");
        }
    }
    ...
    private boolean convertToBoolean(BsonValue bsonValue) {
        ...
        throw new BsonConversionException(
                "Cannot convert " + bsonValue.getBsonType() + " to boolean");
    }
    ...
    private int convertToDate(BsonValue bsonValue) {
        switch (bsonValue.getBsonType()) {
            ...
            case STRING:
                return (int) java.time.LocalDate.parse(
                        bsonValue.asString().getValue()).toEpochDay();
            default:
                throw new BsonConversionException(
                        "Cannot convert " + bsonValue.getBsonType() + " to date");
        }
    }
