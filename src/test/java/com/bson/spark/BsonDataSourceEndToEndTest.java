...
class BsonDataSourceEndToEndTest {
  ...
  private static String testResource(String relativePath) {
    java.net.URL resource =
        BsonDataSourceEndToEndTest.class.getClassLoader().getResource("ejson/" + relativePath);
    if (resource == null) {
      ...
      StructType schema =
          new StructType(
              new StructField[] {
                ...
                new StructField(
                    "decimal_field", DataTypes.createDecimalType(10, 4), true, Metadata.empty()),
  ...
  class SparseFieldsTest {
    ...
    void handlesMissingFieldsAsNull() {
      ...
      Dataset<Row> df =
          spark.read().format("bson").schema(schema).load(testResource("sparse_fields.json"));
    ...
    void supportsSparkSqlQueriesOnSparseData() {
      ...
      List<Row> patients =
          spark.sql("SELECT name FROM sparse_data WHERE resourceType = 'Patient'").collectAsList();
      ...
      List<Row> withAge =
          spark
              .sql("SELECT name, age FROM sparse_data WHERE age IS NOT NULL ORDER BY age")
              .collectAsList();
  ...
  class NestedFederationTest {
    ...
    void supportsNestedFieldProjection() {
      ...
      List<Row> results =
          spark.sql("SELECT id, meta.versionId, identifier FROM federation_data").collectAsList();
  ...
  class FormatAliasTest {
    ...
    void ejsonFormatAliasWorks() {
      ...
      Dataset<Row> df =
          spark.read().format("ejson").schema(schema).load(testResource("sparse_fields.json"));
